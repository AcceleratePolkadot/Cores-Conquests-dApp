  ...
  return (
    <ul className="mx-auto mt-20 grid max-w-2xl grid-cols-1 gap-6 sm:grid-cols-2 lg:mx-0 lg:max-w-none lg:grid-cols-3 lg:gap-8">
      {accounts.map((account) => (
        <li key={account.address} className="rounded-2xl bg-gray-800 px-8 py-10 text-center">
          <button type="button" onClick={() => handleSelectAccount(account)} className="w-full">
            ...
            <h3 className="mt-6 truncate font-semibold text-lg text-white leading-7 tracking-tight">
              {account.name}
            </h3>
            <p className="truncate text-gray-400 text-xs leading-6">{account.address}</p>
          </button>
        </li>
      ))}
    </ul>
